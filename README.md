...
- 검증된 노드의 `ACTION_CLICK`이 실패한 경우에만 그 노드 중앙에 `dispatchGesture()` 탭을 1회 보내며 임의 AI 좌표 제스처는 사용하지 않음
...
Vision이 반환한 좌표 자체로 제스처를 만들지 않습니다. 좌표와 겹치는 실제 접근성 클릭 노드가 정확히 하나이고, 실행 직전 패키지·민감 문맥·노드 path·화면 perceptual fingerprint가 다시 일치할 때만 사용자 확인 후 `ACTION_CLICK`을 한 번 요청합니다. `ACTION_CLICK`이 실패한 경우에만 검증된 동일 노드의 중앙에 `dispatchGesture()` 탭을 한 번 보내며 외부 화면 글자 입력과 임의 좌표 스와이프는 계속 차단합니다.

Gemini 구조 계획으로 성공한 저위험 단일 화면 동작은 같은 명령 해시·패키지·의미 화면 해시에서만 로컬 재사용합니다. 원본 명령, 스크린샷, 전체 화면 구조는 저장하지 않고 클릭 대상 이름·목표 상태처럼 재실행에 필요한 최소 동작만 최대 40개·30일 동안 보관합니다. 재사용 시에도 현재 화면의 안전 정책과 실제 노드를 처음부터 다시 검증합니다.
