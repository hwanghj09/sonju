...
고령층이 스마트폰을 말과 큰 글씨로 쉽게 사용할 수 있도록 돕는 Android AI 접근성 프로토타입입니다. Android 접근성 노드의 버튼 이름·상태·위치 구조를 먼저 사용하고, 접근성으로 질문이나 실행 대상을 찾지 못하면 원본 화면 스크린샷을 Gemini에 보내 시각적으로 찾습니다.

> **현재 테스트 구성:** 클릭 안전검사는 요청에 따라 코드에서 제거되어 있습니다. 접근성 node를 찾으면 `ACTION_CLICK`, 찾지 못하면 Android 11 이상에서 원본 스크린샷을 Gemini로 분석한 좌표 gesture를 사용합니다. 배민 최종 주문 클릭 확인창도 제거되었습니다. 실제 계정·결제·권한·개인정보 화면에서 사용하지 마십시오.
...
- 접근성 node를 우선 사용하고 찾지 못한 경우 Gemini 원본 스크린샷 분석 좌표를 fallback으로 사용
...
- 클릭은 접근성 node의 `ACTION_CLICK`을 먼저 사용하고 node를 찾지 못하면 스크린샷 좌표 gesture를 사용
