...
> **현재 개발 빌드:** 일반 탐색은 자동 실행합니다. 결제·송금·구매의 최종 확정과 개인정보·인증정보 입력, 개인정보 화면의 불투명한 좌표 탭만 실행 직전에 사용자 확인을 요구합니다. 이 빌드는 실기기·Gemini 운영 검증과 Play 정책 검토 전의 프로토타입입니다.
...
일반 앱 탐색과 입력은 기본 허용입니다. 별도 확인은 결제·송금·구매 확정, 개인정보·자격 증명 입력, 민감 화면 좌표 클릭에만 적용합니다. 잘못된 좌표, 비어 있는 입력값, 여러 편집 필드 중 대상을 확정하지 못한 경우, 관찰하지 않은 미래 화면 행동을 한 번에 묶은 계획은 안전 문제가 아니라 실행 정확성 문제로 차단하고 새 계획을 요청합니다.
...
모델은 도구를 직접 호출하지 않고 구조화된 다음 도구를 제안합니다. 로컬 세션이 최종 목표를 덮어쓰지 못하게 고정하고, speculative batch를 한 단계로 줄이며, live 화면과 도구 인자를 검증한 뒤 `SonjuAccessibilityService`가 실행합니다. 자세한 계약은 [자율 에이전트 아키텍처](docs/AUTONOMOUS_AGENT_ARCHITECTURE.md)와 [안전 계약](docs/SAFETY_CONTRACT.md)을 참고하십시오.
...
7. 결제 최종 확정이나 개인정보 입력이 필요한 경우에만 현재 앱 위의 하단 확인 패널에서 목표·도구·입력값을 읽고 직접 승인합니다.
