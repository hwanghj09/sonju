...
현재 코드는 테스트 요청에 따라 클릭의 사전 정책, 사용자 확인, 민감·고위험 화면, 중복 후보, revision/fingerprint, node 상태 및 사후조건 검사를 제거한 상태입니다. 접근성 node를 찾지 못하면 Android 11 이상에서 원본 화면 스크린샷을 Gemini에 전송해 반환된 좌표를 gesture로 누릅니다. 배민 최종 주문 확인창도 제거되어 첫 일치 후보를 즉시 클릭합니다. 아래에 기록된 기존 클릭 안전성 설명과 검증 결과는 이 변경 이전의 이력이며 현재 동작을 보증하지 않습니다. 현재 계약은 `docs/SAFETY_CONTRACT.md`를 기준으로 합니다.
...
- 외부 앱의 임의 클릭과 글자 입력은 의도적으로 차단되어 있습니다. 예외는 배민 패키지의 정적 주문 보조뿐이며, 검색·첫 일치 결과·기본 옵션·장바구니로 제한되고 주문·결제 후보마다 사용자 재확인과 동일 화면 재검증이 필요합니다. 배민 UI 업데이트나 OEM 접근성 트리 차이로 라벨·구조가 바뀌면 중단하는 것이 정상이며 실기기 회귀 검증이 필요합니다. “모바일 폰의 모든 요소”를 포괄적으로 조작하는 제품 요구와 안전·플랫폼·Play 정책은 양립하지 않습니다.
...
- 서비스 강제 종료·업데이트·재부팅·앱 또는 지속 알림의 끄기 동작
